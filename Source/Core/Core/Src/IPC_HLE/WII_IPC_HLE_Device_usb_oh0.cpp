...
void CWII_IPC_HLE_Device_usb_oh0::USBDevicesChanged(USBInterface::TDeviceList& List)
{
...
bool CWII_IPC_HLE_Device_usb_oh0::IOCtlV(u32 _CommandAddress)
{
	...
	case USBV0_ROOT_IOCTLV_GETDEVLIST:
	{
		...
		for (auto it = List.begin(); it != List.end() && Count; ++it)
		{
			...
			{
				OutBuf->Vid = it->idVendor;
				OutBuf->Pid = it->idProduct;
				Count--;
				OutBuf++;
				RealCount++;
			}
...
: IWII_IPC_HLE_Device_usb_x(_rDeviceName)
{
	std::pair<u32, USBInterface::IUSBDevice*> P = OpenVidPid(Vid, Pid, this);
	m_Uid = P.first;
	m_Device = P.second;
}
...
bool CWII_IPC_HLE_Device_usb_oh0_dev::IOCtlV(u32 _CommandAddress)
{
	...
	case USBV0_DEV_IOCTLV_CTRLMSG:
	{
		...
		// this is supposedly little endian
		Setup.wValue = Common::reallyswap16(Memory::Read_U16(CommandBuffer.InBuffer[2].m_Address));
		Setup.wIndex = Common::reallyswap16(Memory::Read_U16(CommandBuffer.InBuffer[3].m_Address));
		Setup.wLength = Common::reallyswap16(Memory::Read_U16(CommandBuffer.InBuffer[4].m_Address));
...
void CWII_IPC_HLE_Device_usb_oh0_dev::USBDevicesChanged(USBInterface::TDeviceList& List)
{
	if (!m_RemovalCommandAddress)
	{
		return;
