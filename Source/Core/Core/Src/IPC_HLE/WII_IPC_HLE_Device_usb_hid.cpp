...
USBInterface::IUSBDevice* CWII_IPC_HLE_Device_usb_hid::GetDevice(u32 DevNum)
{
	...
	USBInterface::IUSBDevice* Device = USBInterface::OpenUid(DevNum, this);
	if (!Device)
	{
...
bool CWII_IPC_HLE_Device_usb_hid::IOCtl(u32 _CommandAddress)
{
	...
	case IOCTL_HID_CONTROL:
	{
		...
		*/
		...
		Setup.bmRequestType = Memory::Read_U8(BufferIn+0x14);
		Setup.bRequest = Memory::Read_U8(BufferIn+0x15);
		Setup.wValue = Memory::Read_U16(BufferIn+0x16);
		Setup.wIndex = Memory::Read_U16(BufferIn+0x18);
		Setup.wLength = Memory::Read_U16(BufferIn+0x1A);
...
bool CWII_IPC_HLE_Device_usb_hid::IOCtlV(u32 _CommandAddress)
{
	...
	Dolphin_Debugger::PrintCallstack(LogTypes::WII_IPC_USB, LogTypes::LWARNING);
	u32 ReturnValue = 0;
	SIOCtlVBuffer CommandBuffer(_CommandAddress);

	DEBUG_LOG(WII_IPC_USB, "%s - IOCtlV:", GetDeviceName().c_str());
	DEBUG_LOG(WII_IPC_USB, "    Parameter: 0x%x", CommandBuffer.Parameter);
	DEBUG_LOG(WII_IPC_USB, "    NumberIn: 0x%08x", CommandBuffer.NumberInBuffer);
	DEBUG_LOG(WII_IPC_USB, "    NumberOut: 0x%08x", CommandBuffer.NumberPayloadBuffer);
	DEBUG_LOG(WII_IPC_USB, "    BufferVector: 0x%08x", CommandBuffer.BufferVector);
	DEBUG_LOG(WII_IPC_USB, "    PayloadAddr: 0x%08x", CommandBuffer.PayloadBuffer[0].m_Address);
	DEBUG_LOG(WII_IPC_USB, "    PayloadSize: 0x%08x", CommandBuffer.PayloadBuffer[0].m_Size);
...
static bool FillAttachedReply(USBInterface::TDeviceList& List, void* Buffer, size_t Size)
{
#define APPEND(Data, DataSize) do { OldPtr = Ptr; if (!AppendRaw(&Ptr, &Size, Data, DataSize)) return false; } while (0)
	...
	for (auto DItr = List.begin(); DItr != List.end(); ++DItr)
	{
		...
		u8* DeviceStart = Ptr;
		u32 Uid = Common::swap32(Device->Uid);
		APPEND(&Dummy, 4); // size
		APPEND(&Uid, 4);
...
void CWII_IPC_HLE_Device_usb_hid::USBDevicesChanged(USBInterface::TDeviceList& List)
{
	...
	bool Result = FillAttachedReply(List, Memory::GetPointer(Buffer), Size);
