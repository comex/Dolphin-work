...
void RefInterface()
{
	...
	{
		// one time setup
		g_PendingDevices = new IntrusiveList<IUSBDevice>;
		g_QueueMutex = new std::mutex;
	...
	{
        DEBUG_LOG(WII_IPC_USB, "USB coming up");
#if defined(__LIBUSB__) || defined (_WIN32)
...
void ResetInterface()
{
	DEBUG_LOG(WII_IPC_USB, "USB shutting down");
...
void RegisterDeviceChangeClient(IUSBDeviceChangeClient* Client)
{
	...
	g_DeviceChangeClients->emplace(Client);
	if (!g_ShouldScan)
	{
		SetShouldScan(true);
		MakeDeviceList();
...
std::pair<u32, IUSBDevice*> OpenVidPid(u16 Vid, u16 Pid, IUSBDeviceClient* Client)
{
	...
	for (auto itr = List.begin(); itr != List.end(); ++itr)
	{
		...
		{
			IUSBDevice* Device = OpenUid(itr->Uid, Client);
			if (Device)
			{
				return std::make_pair(itr->Uid, Device);
			}
		}
	}
	return std::make_pair((u32) 0, (IUSBDevice*) NULL);
}

IUSBDevice* OpenUid(u32 Uid, IUSBDeviceClient* Client)
{
	int ControllerId = Uid >> 28;
	if (ControllerId >= NumControllerIds)
	{
		return NULL;
	}
	IUSBController* Controller = g_Controllers[ControllerId];
	if (!Controller)
	{
		return NULL;
	}
	IUSBDevice* Result = Controller->OpenUid(Uid, Client);
	if (Result)
	{
		g_OpenUids->emplace(Uid);
		MakeDeviceList(); // exclude this
	}
	return Result;
}
